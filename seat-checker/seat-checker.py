...
async def main():
    ...
    while True:
        ...
        for LECTURE_NAME, LECTURE_PROP in LECTURES_TO_FIND.items() :

            #드랍다운 선택
            await set_dropdown_by_index(page, GRADE_DD_ATT, LECTURE_PROP[1])
            
            #여석 가져올 교과목명 입력
            await typeToElementWithWait(page, LECTURE_NAME_INPUT_ATT, LECTURE_NAME)

            #조회 버튼 클릭
            await clickElementWithWait(page, SEARCH_BTN_ATT)

            #전체 강의 수 가져옴
            totalnum = await get_total_lecture_number(page, LECTURE_TOTAL_NUMBER_ATT)
            if totalnum == -1: # 가져오기 실패
                print("전체 강의 수 불러오기 실패")
                continue
            if totalnum == 0: # 조회 시 없는 강의
                print(f"{LECTURE_NAME} : 존재하지 않는 강의")
                continue
            
            #테이블 데이터 가져오기
            lectures = await get_tabledata(page, LECTURE_TABLE_ATT, totalnum)
            if lectures == -1:
            ...
            for lecture in lectures :
                ...
                if delete_whitespace(lecture[2]) == LECTURE_PROP[0] : #lecture[2] : 과목코드-분반
                    ...
                    if remainder != 0:
                        ...
                        if not child_process_started:
                            ...
                            arguments = [LECTURE_NAME, LECTURE_PROP[0]]
